...
module.exports = async function (deployer, network, accounts) { // eslint-disable-line..
  ...
  let enterprise1 = "0x485Be40eBD921c5C759cf7bE6Eeda235cd656E94" // 31
  ...
  console.log("queue no cohort address:", queue.address);

  await deployProxy(QueueCohort, { deployer, initializer: 'initialize' });
  let queueCohort = await QueueCohort.deployed();
  console.log("queue cohort address:", queueCohort.address);
  ...
  // await deployProxy(NodeOperationsHelpers, [], { deployer, initializer: 'initialize' });
  // let nodeOperationsHelpers = await NodeOperationsHelpers.deployed();
  // console.log("node operation helpers address:", nodeOperationsHelpers.address);
  ...
  await memberHelpers.grantRole(CONTROLLER_ROLE, validations.address, { from: admin });
  console.log('memberHelpers.grantRole(CONTROLLER_ROLE, validations.address, { from: admin })')

  await memberHelpers.grantRole(CONTROLLER_ROLE, validationsCohort.address, { from: admin });
  console.log('memberHelpers.grantRole(CONTROLLER_ROLE, validations.address, { from: admin })')
  ...
  0xb598deda69a57e900343fd393089d588a6c52950ee60a8a529d2350403e4e9e7
